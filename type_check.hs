...
alpha_ctx_ren_extend_fresh_ftv =
  Prelude.error "AXIOM TO BE REALIZED"
...
nc_lam :: USort -> String -> Kind -> Term0 -> (List (Prod String Term0)) ->
          NC -> NC
nc_lam =
  Prelude.error "AXIOM TO BE REALIZED"
          ...
          eq_rec_r s0 (\_ ->
            eq_rec_r k (\_ ->
              eq_rec_r s1 (\_ h10 -> Alpha_lam uSort s0 y0 k (sub x t0 s1)
                (sub y t'0 s3) rt0
                (iHs s3 t'0 (nc_lam uSort y0 k s3 (Cons (Pair y t'0) Nil) h3)
                  t0 (nc_lam uSort s0 k s1 (Cons (Pair x t0) Nil) h2) (Cons
            ...
            eq_rec_r k (\_ ->
              eq_rec_r s1 (\_ h9 -> Alpha_lam uSort s0 y k (psubs sigma s1)
                ...
                (iHs (Cons (Pair s0 y) r) s3 sigma sigma'
                  (nc_lam uSort s0 k s1 sigma x)
                  (nc_lam uSort y k s3 sigma' x0) h9
          ...
          eq_rec_r s0 (\_ ->
            eq_rec_r k (\_ ->
              eq_rec_r (sub x t s1) (\_ h14 -> Alpha_lam uSort s0 y k
                ...
                (iHs (nc_lam uSort s0 k s1 (Cons (Pair x t) Nil) h4)
                  (nc_lam uSort s0 k s1 sigma h5)
                  (nc_lam uSort s0 k (psubs sigma s1) (Cons (Pair x
                    (psubs sigma t)) Nil) h7) (Cons (Pair s0 y) r0)
                  (alpha_ctx_ren_extend_fresh_ftv sigma sigma' s0 y r0 h0) s3
                  h14 (nc_lam uSort y k s3 sigma' h3))) s2) a) x7) b __ __ __)
        sigma0 __ __ x8;
     Alpha_app _ _ _ _ _ sigma0 x7 x8 ->
      eq_rec_r r0 (\_ -> false_rec) sigma0 __ __ x7 x8;
     Alpha_builtin r1 _ -> eq_rec_r r0 (\_ -> false_rec) r1 __ __})
          ...
          eq_rec_r x6 (\_ ->
            eq_rec_r a (\_ ->
              eq_rec_r s2 (\_ h10 ->
                ...
                let {
                 iHHstep1 = iHHstep0
                              (nc_lam b x6 a s1 (Cons (Pair x p) Nil) h1)
                              (Cons (Pair x6 y) r0) hctxSub s4 h10
                              (nc_lam b y a s4 (Cons (Pair x p) Nil) h4)}
                in
                case iHHstep1 of {
                 ExistT x9 p0 ->
                  case p0 of {
...
red_beta x s t1 t2 hstep =
  term_rec (\s0 x0 x1 ->
    ...
    let {
     h2 = iHs (nc_lam uSort s0 k s1 (Cons (Pair x t1) Nil) x0)
            (nc_lam uSort s0 k s1 (Cons (Pair x t2) Nil) x1)}
    in
    case h2 of {
     ExistT x2 p ->
      case p of {
   ...
   _evar_0_2 = \_UU0394_ x3 k t _ x4 x5 sigma0 x6 x7 x8 ->
    sn_ty_forall ForAll x3 k (subs sigma0 t)
      (x4 (gu_lam ForAll x3 k t x5) (Cons (Pair x3 (Tmvar x3)) sigma0) __
        (Nc_cons t x3 (Tmvar x3) sigma0 (nc_lam ForAll x3 k t sigma0 x6))
