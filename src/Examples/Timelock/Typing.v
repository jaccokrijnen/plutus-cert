...
Ltac solve_flag :=
  ...
  | |- ?flag = ?flag' -> ?a =>
      try solve 
        [intros Hcon; [discriminate Hcon || eauto with typing]]
  end.
...
Ltac solve__TermBind :=
  match goal with
  | |- ?Delta ,, ?Gamma ;; ?flag |-ok_b (TermBind ?str ?vd ?t) =>
      eapply W_Term; try solve [eauto with typing || repeat__econstructor]
  end.

Ltac solve__DatatypeBind :=
  match goal with
  | |- ?Delta ,, ?Gamma ;; ?flag |-ok_b (DatatypeBind ?dtd) =>
      eapply W_Data; try solve [eauto with typing || solve_flag || solve__constructors || solve__kinding]
  end.
...
Ltac solve__Let :=
  match goal with
  | |- ?Delta ,, ?Gamma ;; ?flag |-+ (Let ?rec ?bs ?t) : ?T =>
      eapply T_Let; try solve [eauto with typing || solve_flag || solve__map_normalise || solve__BindingsNonRec ]
  end.
...
Example pir_0_original_shadowfix__typable :
  empty ,, empty ;; Escape |-+ pir_0_original_shadowfix : (Ty_Fun (Ty_Var "EndDate") (Ty_Fun (Ty_Builtin (Some (TypeIn DefaultUniInteger))) (Ty_Var "Bool"))).
