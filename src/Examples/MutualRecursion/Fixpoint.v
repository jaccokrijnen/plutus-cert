...
(* This could perhaps be an additional rule (too much work now to add it,
...
Definition ty_F_Q_F_Q :=
  <( ∀ ("Q" :: <[ * ]>) "F" "Q" -> "F" "Q")>.

Definition ty_f := <( ("a" -> "b") -> ("a" -> "b"))>.
...
Proof.
  ...
  eapply T_Apply.
  2: {
    repeat constructor. (* all binders *)
    {
      eapply T_Apply.
      2: { constructor. reflexivity. }
      { eapply T_TyInst with "Q" ?[kind_a] ?[ty_0]. (* TODO: we have to pick Q because we're missing alha-equiv*)
        {
          eapply T_Apply with ?[ty_1].
          { apply T_Var.
            reflexivity.
          }
          { repeat constructor.
    ...
    (* Left-over goals are kind-checking *)
    all: repeat econstructor.
  }
  { eapply T_TyInst with "b" Kind_Base ?[ty_1].
    { eapply T_TyInst with "a" Kind_Base ?[ty_2].
      { apply T_TyAbs.
        apply T_TyAbs.
        apply T_LamAbs.
        eapply T_Apply.
        { eapply T_TyInst.
          { apply T_TyAbs.
            apply T_LamAbs.
            2: { repeat econstructor. } (* check kinds *)
            {
              (* Here we need to type-check the term "(unwrap s) s"
                 that is, self application *)
              eapply T_Apply.
              2: { constructor. reflexivity. }

              { eapply T_Unwrap with _ "a" _ _. (* TODO: I have to pick a here, because there is no alpha-equivalence*)
                { eapply T_Normalize.
                ...
                { constructor. reflexivity. }
                {
                  ...
                  eapply Q_Trans.
                  { apply Q_App.
                    { apply Q_Beta. } {apply Q_Refl. }
                  }
                  { simpl.
                    eapply Q_Trans.
                    { apply Q_Beta. } {simpl. apply Q_Refl. }
                  }
                }
              }
            }
      ...
      { simpl. eapply T_Normalize.
        { apply Q_Symm. apply Q_Beta. }
        { eapply T_IWrap with "a" _  _.
          { unfold unwrapIFix.
                apply Q_App.
              { apply Q_Beta. }
              { apply Q_Refl. }
