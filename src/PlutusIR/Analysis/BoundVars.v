...
Inductive appears_bound_in_tm (x : string) : term -> Prop :=
  ...
  | ABI_Tm_Constr_Cons_Head : forall i T t ts,
        appears_bound_in_tm x t ->
        appears_bound_in_tm x (Constr T i (t :: ts))
  | ABI_Tm_Constr_Cons_Tail : forall i T t ts,
        appears_bound_in_tm x (Constr T i ts) ->
        appears_bound_in_tm x (Constr T i (t :: ts))
  | ABI_Tm_Constr_Nil : forall i t T,
        appears_bound_in_tm x t ->
        appears_bound_in_tm x (Constr T i (t :: nil))
...
Inductive appears_bound_in_ann (X : string) : term -> Prop :=
  ...
  | ABI_Ann_Constr_Nil : forall i T t,
      appears_bound_in_ty X T ->
      appears_bound_in_ann X t ->
      appears_bound_in_ann X (Constr T i (t :: nil))
  | ABI_Ann_Constr_Cons : forall i T t ts,
      appears_bound_in_ann X (Constr T i ts) ->
      appears_bound_in_ann X (Constr T i (t :: ts))
...
Function bound_vars (t : term) : list name :=
 match t with
   ...
   | (Constant v)      => []
   | (Builtin f)       => []
   | (Constr T i ts)   => concat (map bound_vars ts)
...
Fixpoint btv (t : term) : list tyname :=
 match t with
   ...
   | Constant v      => []
   | Builtin f       => []
   | (Constr T i ts) => concat (map btv ts)
