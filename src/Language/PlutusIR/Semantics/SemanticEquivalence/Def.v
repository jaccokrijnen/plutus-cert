...
Fixpoint R (T : Ty) (t1 t2 : Term) : Prop :=
  emptyContext |-+ t1 : T /\
  emptyContext |-+ t2 : T /\
  exists v1 v2,
    t1 ==> v1 /\
...
Lemma R_typable_empty : forall T t1 t2,
    R T t1 t2 ->
    emptyContext |-+ t1 : T /\ emptyContext |-+ t2 : T.
Proof. intros. destruct T; destruct H as [Ht1 [Ht2 _]]; auto. Qed.

Lemma R_typable_empty_1 : forall T t1 t2,
    R T t1 t2 ->
    emptyContext |-+ t1 : T.
Proof. intros. destruct (R_typable_empty _ _ _ H). assumption. Qed.

Lemma R_typable_empty_2 : forall T t1 t2,
    R T t1 t2 ->
    emptyContext |-+ t2 : T.
Proof. intros. destruct (R_typable_empty _ _ _ H). assumption. Qed.

Lemma R_evaluable : forall T t1 t2,
    R T t1 t2 ->
    exists v1 v2, t1 ==> v1 /\ t2 ==> v2.
Proof. intros. destruct T; destruct H as [_ [_ [v1 [v2 [Hev1 [Hev2 _]]]]]]; eauto. Qed.

Lemma R_evaluable_1 : forall T t1 t2,
    R T t1 t2 ->
    exists v1, t1 ==> v1.
Proof. intros. destruct (R_evaluable _ _ _ H) as [v1 [_ [Hev1 _]]]. eauto. Qed.

Lemma R_evaluable_2 : forall T t1 t2,
    R T t1 t2 ->
    exists v2, t2 ==> v2.
Proof. intros. destruct (R_evaluable _ _ _ H) as [_ [v2 [_ Hev2]]]. eauto. Qed.

Lemma R_syntactic_equality : forall st t1 t2,
    R (Ty_Builtin st) t1 t2 ->
    exists v1 v2,
      t1 ==> v1 /\
      t2 ==> v2 /\
      v1 = v2.
...
Lemma R_functional_extensionality : forall T1 T2 t1 t2,
  R (Ty_Fun T1 T2) t1 t2 ->
  exists v1 v2,
    t1 ==> v1 /\
    t2 ==> v2 /\
    (forall s1 s2,
...
Lemma R_falsity : forall t1 t2,
  (forall a, ~(R (Ty_Var a) t1 t2)) /\
  (forall bX K T, ~(R (Ty_Lam bX K T) t1 t2)) /\
  (forall T1 T2, ~(R (Ty_App T1 T2) t1 t2)).
Proof. 
  split.
...
Program Fixpoint R'' (T : Ty) (t1 t2 : Term) {measure (countBindingLocations T)} : Prop :=
  ...
  exists v1 v2,
    ...
    | Ty_IFix F T0 => 
        forall K,
          emptyContext |-* T : K ->
          R'' (Named.beta_reduce (unwrapIFix F K T0)) (Unwrap v1) (Unwrap v2)
    |_ => False
    end.
