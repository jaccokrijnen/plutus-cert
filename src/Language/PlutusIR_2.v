...
Notation Term := (term name).
Notation Binding := (binding name).
...
Definition constrBind (d : DTDecl) (c : VDecl) : name * Ty :=
  match d, c with
  | Datatype X YKs matchFunc cs, VarDecl x T =>
    (x, constrTy d c)
  end.
...
Inductive has_type : Context -> Term -> Ty -> Prop :=
  ...
  with constructor_well_formed : Context -> VDecl -> Prop :=
    | W_Con : forall ctx x T,
        (forall U, In U (typeList T) -> ctx |-* U : Kind_Base) ->
        constructor_well_formed ctx (VarDecl x T)
...
Example e : forall t ts,
  test_trace = CompilationTrace t ts ->
  Nil |- t : (Ty_Fun (Ty_Var "EndDate") (Ty_Fun (Ty_Builtin (Some (TypeIn DefaultUniInteger))) (Ty_Var "Bool"))). (* (Ty_Builtin (Some (TypeIn DefaultUniBool))))). *)
Proof.
  intros.
  inversion H. subst.
  clear H.
